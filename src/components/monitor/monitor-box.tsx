...
export default function MonitorBox({ title, icon, value, unit, status = "normal", trend = "stable" }: MonitorBoxProps) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`
                relative w-[280px] p-6 rounded-2xl border-2 ${colors.border} ${colors.bg}
    ...
    >
      ...
      {/* Status Indicator */}
      <div className={`absolute top-4 right-4 w-3 h-3 rounded-full ${colors.accent} animate-pulse`}></div>
      ...
      <div className="mt-6 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500 font-medium">{getTrendIcon()}</span>
        </div>
